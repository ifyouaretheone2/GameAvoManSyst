<template>
  <div class="admin-gameplay">
    <div class="page-header">
      <h1>玩法管理</h1>
      <button @click="showDialog = true" class="btn-primary">添加玩法</button>
    </div>
...
const loadList = async () => {
  try {
    const res = await gameplayApi.getList({ page: 1, pageSize: 100 })
...
const handleSubmit = async () => {
  try {
    if (editingItem.value) {
      await gameplayApi.update(editingItem.value.id, form.value)
    } else {
      await gameplayApi.create(form.value)
    }
